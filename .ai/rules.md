...
- **Every tool advertises an output schema** from `src/growsurf/outputSchemas.ts`, and every success result carries matching `structuredContent` (a tool that declares a schema and omits structured content fails client-side validation). REST tools return their JSON response; guidance and snippet tools return the markdown envelope `{ markdown }` via `markdownToolResult` while keeping the raw document in the text block. A new tool must be added to the schema map and its handler in the same change; a test asserts full coverage.
- When a REST response shape changes, update the matching output schema in the same session. Keep output schemas drift-tolerant (no `required` lists, no `additionalProperties: false`) so additive API changes never fail client-side validation, and document each response shape in full exactly once, on its canonical read tool; sibling tools reference it via `sameShapeAs`.
