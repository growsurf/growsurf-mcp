...
describe("renderMobileSdkGuide", () => {
  it("renders SDK 0.3.3 native window guidance for both mobile platforms", () => {
    ...
    expect(text).toContain("SDK version `0.3.3`");
  ...
  it("emits released public SDK 0.3.3 install coordinates", () => {
    const text = renderMobileSdkGuide(
      {
        platform: "both",
        attributionProvider: "all",
    ...
    expect(text).toContain("/v0.3.3/GrowSurfSDK.podspec");
    expect(text).toContain("com.growsurf:growsurf-android-sdk:0.3.3");
  });
