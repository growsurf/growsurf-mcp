...
describe("renderMobileSdkGuide", () => {
  it("renders SDK 0.5.0 native window guidance for both mobile platforms", () => {
    ...
    expect(text).toContain("SDK version `0.5.0`");
  ...
  it("emits released public SDK 0.5.0 install coordinates", () => {
    const text = renderMobileSdkGuide(
      {
        platform: "both",
        attributionProvider: "all",
    ...
    expect(text).toContain("pod 'GrowSurfSDK', '~> 0.5.0'");
    expect(text).not.toContain("GrowSurfSDK.podspec");
    expect(text).toContain("com.growsurf:growsurf-android-sdk:0.5.0");
  });
