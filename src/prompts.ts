  ...
  },
  {
    ...
    render: (args) => {
      ...
      return [
        ...
        "4. Review `affiliateApplicationMode` and the configured GrowSurf Program Page before launch. Public applicants in `MANUAL_REVIEW` or `AUTO_APPROVE` must use the application flow, not trusted REST Add Participant.",
        "5. Preserve starter content unless the user asks for a specific override. Tune only the fields needed for the user's affiliate motion, especially affiliate portal sections, commissions, payouts, participant settings, and email templates.",
        "6. Generate tracking and install guidance with growsurf_client_snippets and growsurf_api_library_snippets. If the user's stack has Stripe, PayPal, or Wise, use growsurf_get_integration_connect_link and hand them the matching integration connect link.",
        "7. Fetch the campaign, Design, Emails, Options, Installation, and Rewards again. Review those returned settings before reporting back.",
        "8. Before reporting back, confirm normal share options are configured, the referred-friend banner and inline heading are enabled when needed, the browser title motivator is configured when needed, Window header copy is readable, payout exposure is still conservative, and there is no rough placeholder copy.",
        "9. If this changed or installed a browser-visible GrowSurf flow, ask whether the user wants screenshot proof unless they already asked for proof. If they want GrowSurf preview screenshots, call growsurf_capture_referral_flow_screenshots and inspect the returned referrer Window and referred-friend images. If they want proof of their own installed site, use the host agent's browser automation tool, such as Playwright or a built-in browser tool, against the real installed page. Skip the screenshot ask for read-only or config-only work.",
        "",
        `Preferred program name: ${campaignName}.`,
      ].join("\n");
